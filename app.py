# -*- coding: utf-8 -*-
"""
橘貓代購 LINE Bot（Flask 版）
功能：
- 客戶私聊自助報價：報價 1680 / 報價 2200 VIP3 用券
- 商家私聊推播：報價 2200 VIP3 用券 @小美（需在 .env 設 ADMIN_USER_IDS）
- 名單管理：自動記錄「顯示名稱→userId」，或用「綁定 小美 Uxxxxxxxx...」
- 健康檢查：/health 和 /bot/health
- Webhook：/callback 和 /bot/callback（支援 Trim path 與否）
"""
...
# ------------------- 讀取 .env（若環境變數不存在就從 .env 補上） -------------------
def _load_dotenv(path="/home/jumao/linebot/.env"):
    try:
...
# 可設定多個 admin，以逗號分隔（商家自己的 userId）
...
def calc_base_fee(rmb: int) -> int:
    # 你原本的分層：1~499:30；500~999:50；... 每 500 +50；以此類推
    return 30 + (rmb // 500) * 50

def parse_command(text: str):
    """解析客戶自助報價的指令。
       支援：報價 1680、報價1680、報價 2200 VIP3 用券"""
    text = text.strip()
    if not text.startswith("報價"):
        return None
    # 空白改為可選，容忍：全形/半形空白與冒號
...
def set_alias(alias: str, user_id: str):
    alias = alias.strip()
    if not alias or not user_id or not user_id.startswith("U"):
        return False
    with _users_lock:
        d = _load_users()
        d[alias] = user_id
...
def remember_user_profile(user_id: str):
    """客戶第一次私聊時，順手把顯示名稱→userId 記起來，之後可用 @別名 推播。"""
    try:
...
def push_quote_to_alias(rmb: int, level: str, use_coupon: bool, alias: str) -> str:
    ...
    if not uid:
        return f"找不到別名「{alias}」，請先讓對方私聊一次，或用「綁定 {alias} Uxxxxxxxx...」新增。"
